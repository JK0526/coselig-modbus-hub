/* Sunwave Hub management panel. It uses only Home Assistant's public panel API. */
(function () {
  "use strict";
  ...
  class SunwaveModbusHubPanel extends HTMLElement {
    set hass(value) {
      this._hass = value;
    ...
    async _load(force) {
      ...
      try {
        this._data = await this._hass.callWS({ type: "sunwave_modbus_hub/list" });
        this._error = null;
    ...
    _render() {
      ...
      this.innerHTML = `
        ...
        <main>
          ...
          </main>` : `<section class="form"><h2>尚未設定 Hub</h2><p>請先在 Home Assistant 的「設定 → 裝置與服務」加入 Sunwave Modbus Hub。</p></section></main>`}
      `;
      this._bind(entry);
    }
    ...
    _bind(entry) {
      ...
      this.querySelector("#save-poll").addEventListener("click", () => this._call({ type:"sunwave_modbus_hub/set_polling", entry_id:entry.entry_id, enabled:this.querySelector("#polling").checked, interval:Number(this.querySelector("#interval").value) }));
      this.querySelector("#channel-form").addEventListener("submit", (event) => {
        ...
        this._call({ type:"sunwave_modbus_hub/save_channel", entry_id:entry.entry_id, channel:{ model, slave:Number(this.querySelector("#slave").value), channel, kind:(channel === "a" || channel === "b") ? "dual" : "single", name:this.querySelector("#name").value.trim(), minimum:Number(this.querySelector("#minimum").value), mired_min:Number(this.querySelector("#mired_min").value), mired_max:Number(this.querySelector("#mired_max").value) } });
        event.target.reset();
      });
      this.querySelectorAll(".remove").forEach((button) => button.addEventListener("click", () => { if (window.confirm("刪除這個通道並清除其 Discovery 設定？")) this._call({ type:"sunwave_modbus_hub/remove_channel", entry_id:entry.entry_id, slave:Number(button.dataset.slave), channel:button.dataset.channel }); }));
  ...
  customElements.define("sunwave-modbus-hub-panel", SunwaveModbusHubPanel);
})();
